...
namespace VolvoWrench.Demo_Stuff.GoldSource
{
    ...
    public sealed partial class Verification : Form
    {
        ...
        {
            InitializeComponent();
            this.Text = "Verification Scripted";
                ...
                for (int index = 0; index < info.Value.GsDemoInfo.IncludedBXtData[i].Objects.Count; index++)
                {
                    ...
                    switch (t.Key)
                    {
                        ...
                        case Bxt.RuntimeDataType.ALIAS_EXPANSION:
                            {
                                ...
                                {
                                    AppendColored(mrtb, "\t" + "Movement command in alias [" + ((Bxt.AliasExpansion)t.Value).name + "]: " + aliasCommand + " — Frame: " + i + "\n");
                                }
                        ...
                        case Bxt.RuntimeDataType.COMMAND_EXECUTION:
                            {
                                // this is horrendously bad.. HOLY! what was i thinking when writing this code?
                                // this will match random commands if lhs is false and rhs is true, e.g.
                                // r_drawviewmodel, crosshair etc.
                                ...
                                if (command.ToUpper().StartsWith("SAVE"))
                                {
                                    ...
                                    if(!(saveName == "QUICK" || saveName == "HARD" || saveName == "AUTOSAVE"))
                                    {
                                        AppendColored(mrtb, "\t" + command + " — Frame: " + i + "\n");
                                    }
                                }
